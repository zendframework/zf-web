 ...
 * @version    $Id: TestCommon.php 6171 2007-08-21 22:37:18Z bkarwin $
 */
...
abstract class Zend_Db_Adapter_TestCommon extends Zend_Db_TestSetup
{
    ...
    /**
     * Test the adapters fetchAll() against LOB columns
     */
    public function testAdapterFetchLob()
    {
        $documents = $this->_db->quoteIdentifier('zfdocuments');
        $result = $this->_db->fetchAll("SELECT * FROM $documents");
        
        $this->assertEquals(3, count($result)); //count rows
        
        $clobdata = 'this is the clob that never ends...this is the clob that never ends...this is the clob that never ends...';
        $blobdata = 'this is the blob that never ends...this is the blob that never ends...this is the blob that never ends...';
        
        $row = $result[0];
        $this->assertEquals($row['doc_clob'], $clobdata);
        $this->assertEquals($row['doc_blob'], $blobdata);
        
        $row = $result[1];
        $this->assertNull($row['doc_clob']);
        $this->assertNull($row['doc_blob']);
        
        $row = $result[2];
        $this->assertEquals($row['doc_clob'], '');
        $this->assertEquals($row['doc_blob'], '');
    }
