...
class Zend_Cache_Frontend_Page extends Zend_Cache_Core
{
    ...
    public function _flush($data)
    {
        ...
        $memorizeHeaders=array();
        foreach($this->_specificOptions['memorize_headers'] as $headerName)
        {
            $memorizeHeaders[strtolower($headerName)]=$headerName;
        }

        $storedHeaders = array();
        foreach (headers_list() as $headerSent) {
            $tmp=explode(':', $headerSent);
            if(array_key_exists(strtolower($tmp[0]),$memorizeHeaders))
            {
                $storedHeaders[]=$headerSent;
            }
        }
        $array = array(
            'data' => $data,
