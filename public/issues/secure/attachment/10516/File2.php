...
class Zend_Cache_Backend_File2 extends Zend_Cache_Backend implements Zend_Cache_Backend_Interface 
{
	...
	public function save($data, $id, $tags = array(), $specificLifetime = false)
	{
		...
		$dataStore = pack('L', $this->_expireTime($this->getLifetime($specificLifetime)))
		             . $this->_hash($data)
		             . $data;
		...
		// tag file (delete data file on error)
		if (count($tags)) {
	...
	public function load($id, $doNotTestCacheValidity = false) 
	{
		...
		$dataPathname = $this->_lastTestBuffer['path'] . $this->_filename($id, '.data');
		...
		fseek($fp, 4, SEEK_SET);  // first 4 bytes are the expire time
	...
	public function clean($mode = Zend_Cache::CLEANING_MODE_ALL, $tags = array())
	{
		...
		foreach (new RecursiveIteratorIterator($dir, true) as $file) {
			...
			$id = substr($filename, $strlenPrefix, (strrpos($filename, '.')-$strlenPrefix) );
			...
			switch ($mode) {
				case Zend_Cache::CLEANING_MODE_ALL:
					$result = $result && $this->_remove($file->getPathname());
					break;
				
				case Zend_Cache::CLEANING_MODE_OLD:
					if (substr($filename, -5) == '.data') {
						if (!$this->_test($id, false)) {
				...
				default:
					Zend_Cache::throwException("Incorrect clean function : $mode");
			}
		}
	...
	public function _test($id, $doNotTestCacheValidity)
	{
		if ($id != $this->_lastTestBuffer['id']) {
			...
			$dataPathname = $path . $this->_filename($id, '.data');
			if (!file_exists($dataPathname) || !($fp=fopen($dataPathname, 'rb')) ) {
				return false;
			}
			$this->_flock($fp, LOCK_SH);
			// no magic_quotes on reading binary data
			$this->_lastTestBuffer['exp'] = unpack('L*', fread($fp, 4));
			$this->_flock($fp, LOCK_UN);
			fclose($fp);
			
		} elseif ( !$this->_lastTestBuffer['exp'] ) {
			return false;
		}
		
		if ($doNotTestCacheValidity === true) {
			return $this->_lastTestBuffer['exp'];
		}
		
		if (time() <= $this->_lastTestBuffer['exp']) {
			if (!isset($this->_lastTestBuffer['mtime'])) {
				$this->_lastTestBuffer['mtime'] = filemtime($this->_lastTestBuffer['path'] . $this->_filename($id, '.data'));
			}
			return $this->_lastTestBuffer['mtime'];
		}
		return false;
	}
