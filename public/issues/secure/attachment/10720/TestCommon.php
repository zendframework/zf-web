 ...
 * @version    $Id: TestCommon.php 6012 2007-08-05 00:10:00Z bkarwin $
 */
...
abstract class Zend_Db_Adapter_TestCommon extends Zend_Db_TestSetup
{
     ...
     * @return array
     */
    public function _testAdapterDescribeTableCaseCommon($case)
    {
        $params = $this->_util->getParams();
        ...
        return $db->describeTable($products);   
    }
    
    public function testAdapterDescribeTableNaturalCase()
    {
        $desc = $this->_testAdapterDescribeTableCaseCommon(Zend_Db::CASE_NATURAL);
        $product_id = 'product_id';
        $this->assertTrue(array_key_exists($product_id, $desc));
        
        $this->assertEquals($desc[$product_id]['COLUMN_NAME'], $product_id, 'Natural case does not match in describe table');
    }
    
    public function testAdapterDescribeTableUpperCase()
    {
        $desc = $this->_testAdapterDescribeTableCaseCommon(Zend_Db::CASE_UPPER);
        $product_id = 'PRODUCT_ID';
        $this->assertTrue(array_key_exists($product_id, $desc));
    ...
    public function testAdapterDescribeTableLowerCase()
    {
        $desc = $this->_testAdapterDescribeTableCaseCommon(Zend_Db::CASE_LOWER);
        $product_id = 'product_id';
        $this->assertTrue(array_key_exists($product_id, $desc));
        
        $this->assertEquals($desc[$product_id]['COLUMN_NAME'], $product_id, 'Lower case does not match in describe table');
    }
