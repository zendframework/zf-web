...
class Zend_Mail extends Zend_Mime_Message
{
     ...
     * @static
     */
    protected static $_defaultReplyTo = array();
    ...
    */
    ...
    public function addBcc($email, $name='')
    {
        ...
        foreach ($email as $n => $recipient) {
            $this->_addRecipientAndHeader('Bcc', $recipient, is_int($n) ? '' : $n);
        }
	...
	/**
     * Adds Reply-To Header, $email can be an array, or a single string address
     *
     * @param string $email
     * @param string $name
     * @return Zend_Mail Provides fluent interface
    ...
    public function addReplyTo($email, $name = '')
    {
    	...
    	foreach ($email as $n => $replyTo) {
	        $email = $this->_filterEmail($replyTo);
	        $name  = is_int($n) ? '' : $this->_filterName($n);
	        $this->_replyTo[] = $email;
    ...
    /**
     * Returns the Reply-To addresses of the message
     *
     * @return array Reply-To addresses, empty array when not set
     */
    ...
    public function send($transport = null)
    {
        ...
        if(array() === $this->_replyTo && array() !== self::getDefaultReplyTo()) {
