    ...
    public function match($path)
    {
        ...
        for ($i=0, $totalParts=count($this->_parts); $i<$totalParts; $i++) {
            ...
            if ($part['type'] === 'var') { // if the current part is a var try a regex match
                ...
                // if we're not at the last part yet
                if ($i+1 < $totalParts) {
                    // look ahead and see if the next part should be a required or optional match
                    $nextPart = $this->_parts[$i+1];
                    if ($nextPart['type'] === 'var') {
                        // if the next part is a var we can check if it has a default value or not
                    ...
                    } elseif ($nextPart['name'] === '/' && $i+2 < $totalParts &&
                    ...
                    // otherwise this is the last part so there is nothing optional or required to match
                    $nextPart['regex'] = '';
                    $optionalRegex = '';
                }
                ...
                if (preg_match($regex, $path, $matches, PREG_OFFSET_CAPTURE)) {
                    ...
                    if ($matches[2][0] !== '') {
                        ...
                        if ($nextPart['type'] === 'var') {
                            ...
                            $offset += strlen($nextPart['name']); // otherwise it is a path and we can skip past it
                        }
                        $i++;
                    }
                ...
                if ($part['name'] === substr($path, $offset, $nameLength)) {
                    $offset = $nameLength;
                } elseif ($part['name'] === '/' && $i+1 < $totalParts &&
        ...
        if ($this->_captureParams && $path !== false) {
