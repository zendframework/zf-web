...
if ($this->_parts[$pos] == '*') {
  ...
  for($i = $pos; $i < $count; $i+=2) {
    ...
    if ($this->_isTranslated) {
        if (($originalPathPart = array_search($var, $translateMessages)) !== false) {
            $var = $originalPathPart;
        }
    }
    ...
    if (!isset($this->_wildcardData[$var]) && !isset($this->_defaults[$var]) && !isset($values[$var])) {
      ...
      if (($originalPathPart = array_search($value, $translateMessages)) !== false) {
          $value = $originalPathPart;
      }
      
      $this->_wildcardData[$var] = $value;
    }
  }
...
// PATCH (Start)
if ($this->_isTranslated) {
  $value = $translator->translate($this->_wildcardData[$name], $locale);
} else {
...
if (!$reset) {
  if ($this->_isTranslated) {
    foreach($data as $var => $value) {
      ...
      $var = $translator->translate($var, $locale);
