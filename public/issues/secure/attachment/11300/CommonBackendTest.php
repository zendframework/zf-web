...
class Zend_Cache_CommonBackendTest extends PHPUnit_Framework_TestCase {
	...
	$this->config_ini=parse_ini_file(dirname(dirname($this->_root)).DIRECTORY_SEPARATOR. 'config.ini');
    ...
    {
    	//check whether the path given in config.ini is writeable
	if (is_writeable($this->config_ini['tmpdir'])){
	    return $this->config_ini['tmpdir'].DIRECTORY_SEPARATOR . 'zend_cache_tmp_dir';
	// if not, is TMPDIR exported?
